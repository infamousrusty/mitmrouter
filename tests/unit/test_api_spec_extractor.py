...
class TestAPISpecExtractor:
    def test_tracks_server(self, sample_http_flow: Any) -> None:
    ...
    def test_adds_path_and_method(self, sample_http_flow: Any) -> None:
        extractor = APISpecExtractor()
        extractor.response(sample_http_flow)
        assert "/v1/users" in extractor._spec["paths"]
        assert "get" in extractor._spec["paths"]["/v1/users"]

    def test_infers_response_schema(self, sample_http_flow: Any) -> None:
        extractor = APISpecExtractor()
        extractor.response(sample_http_flow)
        op = extractor._spec["paths"]["/v1/users"]["get"]
        assert "200" in op["responses"]

    def test_schema_inference_dict(self) -> None:
        extractor = APISpecExtractor()
    ...
    def test_schema_inference_list(self) -> None:
        extractor = APISpecExtractor()
        schema = extractor._infer_schema([{"id": 1}])
        assert schema["type"] == "array"
        assert schema["items"]["type"] == "object"

    def test_export_writes_json_without_yaml(
        self,
        tmp_path: Path,
