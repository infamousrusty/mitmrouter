...
test_integration() {
    ...
    for lib_file in "${MITMROUTER_DIR}"/lib/*.sh; do
        ...
        if bash -n "${lib_file}" 2>/dev/null; then
            test_assert "true" "${lib_name} has valid syntax"
        else
            test_assert "false" "${lib_name} has valid syntax"
        fi
    done
}
...
test_version() {
    ...
    echo "=========================================="
    local version
    version=$(grep 'MITMROUTER_VERSION=' "${MITMROUTER_DIR}/mitmrouter.sh" | head -n1 | cut -d'"' -f2)
...
run_all_tests() {
    ...
    if [[ ${TESTS_FAILED} -gt 0 ]]; then
        echo ""
        # COLOR_YELLOW used here for the warning header
        echo -e "${COLOR_YELLOW}Failed Tests:${COLOR_NC}"
