...
classify_active_flows() {
    ...
    netstat -tn 2>/dev/null | awk '/ESTABLISHED/ {print $5}' | while read -r remote_addr; do
        ...
        classification=$(match_classification_rule "${host}" "${port}")
...
list_classification_rules() {
    ...
    grep -v '^#' "${rules_file}" | grep -v '^$' | while IFS='|' read -r pattern protocol device _tags; do
...
export_classifications() {
    local format="${1:-json}"
    local output_file
    output_file="${EVIDENCE_DIR}/classifications_$(date +%Y%m%d_%H%M%S).${format}"

    case "${format}" in
        json)
            echo "[" > "${output_file}"
            local first=true
            while IFS='|' read -r ts remote_addr classification; do
...
EOF
        ...
        csv)
            ...
            tr '|' ',' < "${CLASSIFIER_STATE_DIR}/flows.log" >> "${output_file}"
            ;;
    esac
